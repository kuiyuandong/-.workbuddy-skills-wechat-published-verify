...
2. `Win+R` 运行（**推荐用 Edge 默认档案**，登录态本就存在，免重新扫码）：
   ```
   msedge --remote-debugging-port=9222
   ```
   （若默认档案有干扰，再退而用全新临时实例：`msedge --remote-debugging-port=9222 --user-data-dir="C:\Temp\edge-debug"`，但需重新登录微信后台。）
   > 经验配方：本 skill 实测跑通的一次，就是靠技能自带的 `start_edge_cdp.ps1` 以 **Edge 默认档案 + 9222** 启动，微信登录 cookie 自动保留，直接进后台带 token。
...
| Chrome 带参启动后端口仍不监听（双击快捷方式改了参数也没用） | **Chrome 是单实例应用**：只要系统里已有一个 Chrome 在跑，新带的 `--remote-debugging-port` 会被旧实例吞掉丢弃 | 先 `任务管理器` 结束**所有** `chrome.exe`（关窗口不够），再双击带参快捷方式或 `Win+R` 跑；且**不要加 `--user-data-dir` 临时目录**（会丢微信登录态），用默认档案重启即可 |
...
- **Chrome 单实例是个深坑**：想用 Chrome 抓取时，若 Chrome 已在运行，新启动的 `--remote-debugging-port` 参数会被旧实例无声吞掉、端口永远不开。必须先杀光所有 `chrome.exe` 再启动。相比之下 **Edge + 默认档案**更稳（无单实例干扰、登录态保留），本次就是靠它一次跑通。
